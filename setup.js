...
async function updateName(newId) {
  ...
  const indexUpdatedHtml = indexHtml.replace(
    /<div\s+[^>]*data-version[^>]*/i,
    `<div data-version="" id="${newId}"`
  );
  ...
  if (fs.existsSync(envPath)) {
    ...
    envContent = envContent.replace(
      /VITE_APP_BASE_URL=.*/i,
      `VITE_APP_BASE_URL='${baseUrl}/stage'`
    );
  } else {
  ...
  if (fs.existsSync(envProdPath)) {
    ...
    envProdContent = envProdContent.replace(
      /VITE_APP_BASE_URL=.*/i,
      `VITE_APP_BASE_URL='${baseUrl}/release'`
    );
  } else {
